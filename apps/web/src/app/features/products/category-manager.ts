    ...
    @if (showForm()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" (click)="showForm.set(false)">
        <div class="w-96 rounded-lg bg-white p-6 shadow-xl" (click)="$event.stopPropagation()">
          <h3 class="mb-4 text-lg font-semibold">Nueva Categor\u00eda</h3>
          <input #nameInput placeholder="Nombre de la categor\u00eda" class="mb-4 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
...
export class CategoryManager {
  ...
  readonly created = output<Categoria[]>();

  create(name: string): void {
    if (!name.trim()) return;
    this.productService.createCategory({ nombre: name }).subscribe(() => {
      this.showForm.set(false);
      this.productService.listCategories().subscribe((cats) => this.created.emit(cats));
    });
  }
}