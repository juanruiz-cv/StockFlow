  ...
  template: `
    <div class="mx-auto max-w-2xl space-y-6">
      <div class="flex items-center justify-between">
        ...
        <a routerLink="/admin/roles" class="text-sm text-blue-600 transition-colors duration-150 hover:text-blue-800">&larr; Volver</a>
      </div>
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4 rounded-lg border border-gray-200 bg-white p-6">
          ...
          @if (loadingPermissions()) {
            ...
            <div class="space-y-1 rounded-lg border border-gray-200 p-4">
              @for (perm of permissions(); track perm.id) {
                <label class="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm transition-colors duration-150 hover:bg-gray-50">
                  <input
                    type="checkbox"
                    [value]="perm.id"
        ...
        <div class="flex justify-end gap-3">
          <a routerLink="/admin/roles" class="cursor-pointer rounded-lg border border-gray-300 px-4 py-2 text-sm transition-colors duration-150 hover:bg-gray-50">Cancelar</a>
          <button
            type="submit"
            [disabled]="submitting()"
